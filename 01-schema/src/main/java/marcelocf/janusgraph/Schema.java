...
public class Schema {
  ...
  /**
   * The configuration file path relative to the execute path of this code. <br/>
   *
  ...
  /**
   * The index backend is identified by a key in the configuration; in our example
   * we called it
   * 
   * <pre>
   * search
  ...
  /**
   * This <i>is</i> our graph database. Instead of connecting to a remote server,
   * JanusGraph has a built-in implementation of the graph and connects to the
   * backend and indexing databases. <br/>
   * Of course, you can connect to a <i>gremlin server</i> in your code, so you
   * use JanusGraph in a server mode. But this is not what we are doing here.
   */
  private final JanusGraph graph;

  /**
   * The schema management is done by an instance
   * of @{@link JanusGraphManagement}. This class can do other interesting stuff,
   * such as kicking other nodes from the cluster. I recommend reading its
   * javadocs.
   */
  ...
  /**
   * Create both <i>posts</i> and <i>follows</i> edges and related index. <br/>
   *
   * Because the property and index for both follows and posts is the same we
   * create them at the same point here.
   */
  private void createEdgeSchema() {
    ...
    mgt.buildIndex(indexName(POSTS, CREATED_AT), Edge.class).addKey(createdAt).indexOnly(posts)
  ...
  /**
   * We are using this to create predictable names for our indexes. You could name
   * it however you want, but doing like this will make it possible to reindex
   * stuff in the future... if we want (we do want, btw)
   * 
   * @param label
   *          edge or vertex label
   * @param propertyKey
   *          property key
   * @return
   */
